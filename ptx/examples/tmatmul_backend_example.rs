...
fn example_simple_ops() {
    ...
    // Sigmoid activation
    codegen
        .emit_operation("tmatmul.sig", &["%1"], &["%2"])
        .unwrap();

    // Store output
    codegen
        .emit_operation("tmatmul.sv", &["%2", "output"], &[])
        .unwrap();
...
fn example_mlgru_layer() {
    ...
    codegen
        .emit_operation("tmatmul.tmatmul_import", &["%0"], &[])
        .unwrap();
    codegen
        .emit_operation("tmatmul.tmatmul_go", &["WG"], &[])
        .unwrap();
    ...
    // Export G_t
    codegen
        .emit_operation("tmatmul.tmatmul_export", &[], &["%4"])
        .unwrap();

    // Normalize G_t
    codegen
        .emit_operation("tmatmul.norm", &["%4"], &["%4"])
        .unwrap();
    ...
    // v5 = H_t * sig(H_t)
    codegen
        .emit_operation("tmatmul.mul", &["%1", "%2"], &["%5"])
        .unwrap();

    // v5 = norm(G_t) * H_t * sig(H_t)
    codegen
        .emit_operation("tmatmul.mul", &["%4", "%5"], &["%5"])
        .unwrap();

    // Normalize and project through WO
    codegen
        .emit_operation("tmatmul.norm", &["%5"], &["%5"])
        .unwrap();
    codegen
        .emit_operation("tmatmul.matmul", &["%5", "WO"], &["%5"])
        .unwrap();

    // Residual connection
    codegen.add_comment("Residual Connection #1");
    codegen
        .emit_operation("tmatmul.ldv", &["TEMP_VEC"], &["%0"])
        .unwrap();
    codegen
        .emit_operation("tmatmul.add", &["%0", "%5"], &["%0"])
        .unwrap();
...
fn example_glu() {
    ...
    codegen
        .emit_operation("tmatmul.matmul", &["%0", "WU1"], &["%2"])
        .unwrap();

    codegen.add_comment("Up-projection 2: up2 = x * WU2");
    codegen
        .emit_operation("tmatmul.matmul", &["%0", "WU2"], &["%3"])
        .unwrap();

    codegen.add_comment("Gating: gated = silu(up1) * up2");
    codegen
        .emit_operation("tmatmul.silu", &["%2"], &["%2"])
        .unwrap();
    codegen
        .emit_operation("tmatmul.mul", &["%2", "%3"], &["%2"])
        .unwrap();

    codegen.add_comment("Down-projection: output = gated * WN");
    codegen
        .emit_operation("tmatmul.norm", &["%2"], &["%2"])
        .unwrap();
    codegen
        .emit_operation("tmatmul.matmul", &["%2", "WN"], &["%2"])
        .unwrap();
    ...
    codegen
        ...
        .unwrap();
    codegen
        .emit_operation("tmatmul.add", &["%0", "%2"], &["%0"])
        .unwrap();
