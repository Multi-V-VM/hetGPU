...
// Must match Gemmini hardware configuration (DIM = 16)
pub const TENSOR_BATCH_DIM_X: i64 = 16;
pub const TENSOR_BATCH_DIM_Y: i64 = 16;
...
#[derive(Debug, Clone, PartialEq)]
struct TensorType {
    dims: Vec<i64>,    // Flexible dimensions list
...
impl std::fmt::Display for TensorType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if self.dims.is_empty() {
            // Scalar tensor
            write!(f, "tensor<{}>", self.ty)
        } else {
            // Format as tensor<d1xd2x...xdnxtype>
            let dims_str = self.dims.iter()
                .map(|d| d.to_string())
                .collect::<Vec<_>>()
                .join("x");
            write!(f, "tensor<{}x{}>", dims_str, self.ty)
        }
    }
}
...
impl<'a, 'input> PtxToTosaConverter<'a, 'input> {
    ...
    fn get_param_type_for_function(&self, func_name: &str) -> MlirType {
        match self.get_function_category(func_name) {
            FunctionCategory::Bitwise => MlirType::Tensor(TensorType {
                dims: vec![TENSOR_BATCH_DIM_X, TENSOR_BATCH_DIM_Y],
                ty: BasicType::I32,
            }),
    ...
    fn convert_instruction(
        ...
        match inst {
            ...
            ast::Instruction::Call { data, .. } => {
                eprintln!("ZLUDA DEBUG: Converting CALL instruction!");
                Ok(Some(self.convert_call_instruction(data)?))
            }
            _ => {
    ...
    fn convert_mul_instruction(
        ...
        let mut use_matmul = {
            ...
            eprintln!("ZLUDA DEBUG: Matmul detection - src1_ssa: {}, src2_ssa: {}", src1_ssa, src2_ssa);
            eprintln!("ZLUDA DEBUG: src1_is_arg: {}, src2_is_arg: {}", src1_is_arg, src2_is_arg);
            
            // Use matmul when both inputs are function arguments (indicating matrix inputs)
            // and they are different arguments
            src1_is_arg && src2_is_arg && (src1_ssa != src2_ssa)
        };
        ...
        if use_matmul {
            ...
            self.ssa_types
                .insert(matmul_result_ssa.clone(), src_tensor_type.clone());
            self.value_map.insert(dst, matmul_result_ssa.clone());
            
            return Ok(matmul_result_ssa);
        }

        // Original element-wise multiplication code
        ...
        // TOSA mul always requires 3 operands: input1, input2, shift
        // For float, shift is ignored but still required
    ...
    fn convert_activemask_instruction(&mut self, dst: SpirvWord) -> Result<String, TranslateError> {
        ...
        // For activemask, return a constant tensor with value 1 (indicating single active thread)
        // activemask.b32 returns a 32-bit integer mask
        self.write_line(&format!(
        ...
        self.value_map.insert(dst, dst_ssa.clone());
        eprintln!("ZLUDA DEBUG: Generated activemask instruction returning integer value 1");
        Ok(dst_ssa)
    }
    ...
    fn convert_mad_instruction(
        ...
        if is_float {
            ...
            // TOSA mul always needs shift parameter (ignored for float)
            let shift_ssa = self.next_ssa_value();
            let shift_type_str = self.get_tosa_shift_tensor_type();
            self.write_line(&format!(
                "{} = \"tosa.const\"() {{values = dense<0> : {}}} : () -> {}",
    ...
    fn convert_setp_instruction(
        ...
        // Get the predicate tensor type string once for reuse
        let pred_type = self.get_predicate_tensor_type_str();
        ...
        match data.cmp_op {
            ...
            | ast::SetpCompareOp::Float(ast::SetpCompareFloat::Eq) => {
                self.write_line(&format!(
                    "{} = \"tosa.equal\"({}, {}) : ({}, {}) -> {}",
                    dst_ssa, src1_ssa, src2_ssa, tensor_type, tensor_type, pred_type
                ));
            }
            ...
            | ast::SetpCompareOp::Float(ast::SetpCompareFloat::NotEq) => {
                ...
                self.write_line(&format!(
                    "{} = \"tosa.equal\"({}, {}) : ({}, {}) -> {}",
                    temp_ssa, src1_ssa, src2_ssa, tensor_type, tensor_type, pred_type
                ));
                self.write_line(&format!(
                    "{} = \"tosa.logical_not\"({}) : ({}) -> {}",
                    dst_ssa, temp_ssa, pred_type, pred_type
                ));
            }
            ...
            | ast::SetpCompareOp::Float(ast::SetpCompareFloat::Less) => {
                self.write_line(&format!(
                    "{} = \"tosa.greater\"({}, {}) : ({}, {}) -> {}",
                    dst_ssa, src2_ssa, src1_ssa, tensor_type, tensor_type, pred_type
                ));
            }
            ...
            ast::SetpCompareOp::Float(ast::SetpCompareFloat::NanEq) => {
                ...
                self.write_line(&format!(
                    "{} = \"tosa.equal\"({}, {}) : ({}, {}) -> {}",
                    eq_ssa, src1_ssa, src2_ssa, tensor_type, tensor_type, pred_type
                ));

                // Check if src1 is NaN (x != x)
                self.write_line(&format!(
                    "{} = \"tosa.equal\"({}, {}) : ({}, {}) -> {}",
                    nan1_ssa, src1_ssa, src1_ssa, tensor_type, tensor_type, pred_type
                ));
                self.write_line(&format!(
                    "{} = \"tosa.logical_not\"({}) : ({}) -> {}",
nan1_not_ssa, nan1_ssa, pred_type, pred_type
                ));

                // Check if src2 is NaN
                self.write_line(&format!(
                    "{} = \"tosa.equal\"({}, {}) : ({}, {}) -> {}",
                    nan2_ssa, src2_ssa, src2_ssa, tensor_type, tensor_type, pred_type
                ));
                self.write_line(&format!(
                    "{} = \"tosa.logical_not\"({}) : ({}) -> {}",
nan2_not_ssa, nan2_ssa, pred_type, pred_type
                ));

                // Result is true if equal OR either is NaN
                self.write_line(&format!(
                    "{} = \"tosa.logical_or\"({}, {}) : ({}, {}) -> {}",
                    or1_ssa, eq_ssa, nan1_not_ssa, pred_type, pred_type, pred_type
                ));
                self.write_line(&format!(
                    "{} = \"tosa.logical_or\"({}, {}) : ({}, {}) -> {}",
                    dst_ssa, or1_ssa, nan2_not_ssa, pred_type, pred_type, pred_type
                ));
            }
            ast::SetpCompareOp::Float(ast::SetpCompareFloat::NanNotEq) => {
                ...
                self.write_line(&format!(
                    "{} = \"tosa.logical_not\"({}) : ({}) -> {}",
                    neq_ssa, eq_ssa, pred_type, pred_type
                ));
...
nan2_not_ssa, nan2_ssa, pred_type, pred_type
                ...
                self.write_line(&format!(
                    "{} = \"tosa.logical_or\"({}, {}) : ({}, {}) -> {}",
                    or1_ssa, neq_ssa, nan1_not_ssa, pred_type, pred_type, pred_type
                ));
                self.write_line(&format!(
                    "{} = \"tosa.logical_or\"({}, {}) : ({}, {}) -> {}",
                    dst_ssa, or1_ssa, nan2_not_ssa, pred_type, pred_type, pred_type
            ...
            ast::SetpCompareOp::Float(ast::SetpCompareFloat::NanLess) => {
                ...
                self.write_line(&format!(
                    "{} = \"tosa.greater\"({}, {}) : ({}, {}) -> {}",
                    lt_ssa, src2_ssa, src1_ssa, tensor_type, tensor_type, pred_type
                ));
...
nan2_not_ssa, nan2_ssa, pred_type, pred_type
                ...
                self.write_line(&format!(
                    "{} = \"tosa.logical_or\"({}, {}) : ({}, {}) -> {}",
                    or1_ssa, lt_ssa, nan1_not_ssa, pred_type, pred_type, pred_type
                ));
                self.write_line(&format!(
                    "{} = \"tosa.logical_or\"({}, {}) : ({}, {}) -> {}",
                    dst_ssa, or1_ssa, nan2_not_ssa, pred_type, pred_type, pred_type
            ...
            ast::SetpCompareOp::Float(ast::SetpCompareFloat::NanLessOrEq) => {
                ...
                self.write_line(&format!(
                    "{} = \"tosa.greater_equal\"({}, {}) : ({}, {}) -> {}",
                    le_ssa, src2_ssa, src1_ssa, tensor_type, tensor_type, pred_type
                ));
...
nan2_not_ssa, nan2_ssa, pred_type, pred_type
                ...
                self.write_line(&format!(
                    "{} = \"tosa.logical_or\"({}, {}) : ({}, {}) -> {}",
                    or1_ssa, le_ssa, nan1_not_ssa, pred_type, pred_type, pred_type
                ));
                self.write_line(&format!(
                    "{} = \"tosa.logical_or\"({}, {}) : ({}, {}) -> {}",
                    dst_ssa, or1_ssa, nan2_not_ssa, pred_type, pred_type, pred_type
            ...
            ast::SetpCompareOp::Float(ast::SetpCompareFloat::NanGreater) => {
                ...
                self.write_line(&format!(
                    "{} = \"tosa.greater\"({}, {}) : ({}, {}) -> {}",
                    gt_ssa, src1_ssa, src2_ssa, tensor_type, tensor_type, pred_type
                ));
...
nan2_not_ssa, nan2_ssa, pred_type, pred_type
                ...
                self.write_line(&format!(
                    "{} = \"tosa.logical_or\"({}, {}) : ({}, {}) -> {}",
                    or1_ssa, gt_ssa, nan1_not_ssa, pred_type, pred_type, pred_type
                ));
                self.write_line(&format!(
                    "{} = \"tosa.logical_or\"({}, {}) : ({}, {}) -> {}",
                    dst_ssa, or1_ssa, nan2_not_ssa, pred_type, pred_type, pred_type
            ...
            ast::SetpCompareOp::Float(ast::SetpCompareFloat::NanGreaterOrEq) => {
                ...
                self.write_line(&format!(
                    "{} = \"tosa.greater_equal\"({}, {}) : ({}, {}) -> {}",
                    ge_ssa, src1_ssa, src2_ssa, tensor_type, tensor_type, pred_type
                ));
...
nan2_not_ssa, nan2_ssa, pred_type, pred_type
                ...
                self.write_line(&format!(
                    "{} = \"tosa.logical_or\"({}, {}) : ({}, {}) -> {}",
                    or1_ssa, ge_ssa, nan1_not_ssa, pred_type, pred_type, pred_type
                ));
                self.write_line(&format!(
                    "{} = \"tosa.logical_or\"({}, {}) : ({}, {}) -> {}",
                    dst_ssa, or1_ssa, nan2_not_ssa, pred_type, pred_type, pred_type
            ...
            ast::SetpCompareOp::Float(ast::SetpCompareFloat::IsNotNan) => {
                ...
                self.write_line(&format!(
                    "{} = \"tosa.equal\"({}, {}) : ({}, {}) -> {}",
                    not_nan1_ssa, src1_ssa, src1_ssa, tensor_type, tensor_type, pred_type
                ));

                // Check if src2 is NaN
                self.write_line(&format!(
                    "{} = \"tosa.equal\"({}, {}) : ({}, {}) -> {}",
                    not_nan2_ssa, src2_ssa, src2_ssa, tensor_type, tensor_type, pred_type
                ));

                // Result is true if NEITHER is NaN
                self.write_line(&format!(
                    "{} = \"tosa.logical_and\"({}, {}) : ({}, {}) -> {}",
                    dst_ssa, not_nan1_ssa, not_nan2_ssa, pred_type, pred_type, pred_type
                ));
            }
...
nan2_not_ssa, nan2_ssa, pred_type, pred_type
                ...
                self.write_line(&format!(
                    "{} = \"tosa.logical_or\"({}, {}) : ({}, {}) -> {}",
                    dst_ssa, nan1_not_ssa, nan2_not_ssa, pred_type, pred_type, pred_type
                ));
            }
            _ => {
                return Err(TranslateError::UnknownSymbol);
    ...
    fn convert_selp_instruction(
        ...
        self.write_line(&format!(
            "{} = \"tosa.select\"({}, {}, {}) : ({}, {}, {}) -> {}",
            dst_ssa, src3_ssa, src1_ssa, src2_ssa, pred_type, tensor_type, tensor_type, tensor_type
        ));
    ...
    fn convert_call_instruction(
        ...
        eprintln!(
            "ZLUDA DEBUG: Call instruction - uniform: {}, return_args: {:?}, input_args: {:?}",
            data.uniform, data.return_arguments, data.input_arguments
        );

        // For activemask call pattern:
        // - 1 return argument of type U32
        // - 0 input arguments
        if data.return_arguments.len() == 1 && data.input_arguments.is_empty() {
            if let Some((ast::Type::Scalar(ast::ScalarType::U32), _)) =
                data.return_arguments.first()
            {
                eprintln!("ZLUDA DEBUG: Detected activemask call signature");
                // Since we don't have access to the actual destination from CallDetails,
                // we'll need to handle this differently
                let result_ssa = self.next_ssa_value();
                let int_tensor_type = self.get_integer_tensor_type();

                self.write_line(&format!(
                    "{} = \"tosa.const\"() {{values = dense<1> : {}}} : () -> {}",
                    result_ssa, int_tensor_type, int_tensor_type
                ));

                // Store in last_result_ssa so it can be used by subsequent store
                self.last_result_ssa = Some(result_ssa.clone());

                return Ok(result_ssa);
            }
        }

        self.write_line("// Unsupported function call");
        Ok(String::new())
    }
        ...
        match typ {
            ...
            ast::Type::Array(_, scalar_type, dimensions) => {
                ...
                Ok(MlirType::Tensor(TensorType {
                    dims: vec![TENSOR_BATCH_DIM_X, TENSOR_BATCH_DIM_Y, total_size],
    ...
    fn get_vector_tensor_type(&self, len: u8, scalar_type: ast::ScalarType) -> MlirType {
        MlirType::Tensor(TensorType {
            dims: vec![TENSOR_BATCH_DIM_X, TENSOR_BATCH_DIM_Y, len as i64],
            ty: Self::ptx_scalar_to_basic_type(scalar_type),
        })
    }
    ...
    fn get_tosa_shift_tensor_type(&self) -> String {
        // TOSA mul shift parameter must be a 1D scalar tensor
        format!("tensor<1x{}>", BasicType::I8)
    }

    fn get_predicate_tensor_type(&self) -> MlirType {
        MlirType::Tensor(TensorType {
            dims: vec![TENSOR_BATCH_DIM_X, TENSOR_BATCH_DIM_Y],
            ty: BasicType::I1,
        })
    ...
    fn get_predicate_tensor_type_str(&self) -> String {
        self.get_predicate_tensor_type().to_string()
    }
