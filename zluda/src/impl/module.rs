...
pub(crate) fn load_data(module: &mut CUmodule, image: *const std::ffi::c_void) -> CUresult {
    ...
    if is_binary {
        ...
        if result != ze_result_t::ZE_RESULT_SUCCESS || context.0.is_null() || device.0.is_null() {
            eprintln!("[hetGPU] Binary module load failed or virtual device (result={:?}, ctx_null={}, dev_null={})",
                     result, context.0.is_null(), device.0.is_null());
            eprintln!("[hetGPU] Creating placeholder module - kernel launches will be no-ops");
